...
| `tools/windows/system/setup-kokoro-docker.ps1` | Create/start a local Kokoro TTS container reachable at `http://127.0.0.1:8880`. GPU-accelerated by default (`v0.8.0-cu128`, needed for Blackwell/RTX 50-series — the plain `:latest` tag lacks sm_120 kernels); `-Cpu` avoids display-GPU contention entirely. Previews by default. **Day-to-day management now lives in the sibling `docker-local/kokoro` repo** — this copy stays in sync for the diagnose/quiet tools above | `-Cpu`, `-GpuTag`, `-Port`, `-ContainerName`, `-Apply`, `-Undo` |
