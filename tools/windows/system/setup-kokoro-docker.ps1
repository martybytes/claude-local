...
.PLATFORM    windows
.CATEGORY    system
.USAGE       .\tools\windows\system\setup-kokoro-docker.ps1 [-Cpu] [-GpuTag <tag>] [-Port <int>] [-ContainerName <name>] [-Apply] [-Undo]
.WHEN        User wants Kokoro TTS running locally via Docker, or gpu-tts-diagnose/gpu-tts-quiet has nothing to detect/manage because no TTS container exists yet.
#>
param(
    [switch]$Cpu,                    # use ghcr.io/remsky/kokoro-fastapi-cpu instead of the GPU image (avoids display-GPU contention; see cursor-stall-gpu-tts-runbook.md)
...
if (-not $Undo -and -not $Cpu -and (Have 'nvidia-smi')) {
    ...
    Info 'this container will share the display GPU — see cursor-stall-gpu-tts-runbook.md if the desktop starts stuttering'
}
...
if ($Undo) {
    ...
    if ($existingLine) {
        ...
        $runArgs = @('run', '-d', '--name', $ContainerName, '--restart', 'unless-stopped') + $gpuArgs + @('-p', "${Port}:8880", $image)
